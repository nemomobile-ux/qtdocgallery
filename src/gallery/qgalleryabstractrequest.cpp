...
void QGalleryAbstractRequestPrivate::_q_finished()
{
    if (state == QGalleryAbstractRequest::Active
        ...
        if (response->error() != QGalleryAbstractRequest::NoError) {
            ...
            Q_EMIT q_func()->error(error, errorString);
            Q_EMIT q_func()->errorChanged();
            Q_EMIT q_func()->stateChanged(state);
...
void QGalleryAbstractRequestPrivate::_q_progressChanged(int current, int maximum)
{
    ...
    Q_EMIT q_func()->progressChanged(current, maximum);
}
...
QGalleryAbstractRequest::QGalleryAbstractRequest(QGalleryAbstractRequestPrivate &dd, QObject *parent)
    : QObject(parent)
    , d_ptr(&(dd))
{
    d_ptr->q_ptr = this;
...
void QGalleryAbstractRequest::execute()
{
    ...
    Q_EMIT stateChanged(d_ptr->state);
}
